...
Use this script to produce a T-SQL script to run on the secondary instances of a logshipping configuration failover to it.
*/
...
WHERE sjh.step_id = 0 
      ...
      AND td.database_name IS NOT NULL --not sure why this is here
...
WHILE EXISTS (SELECT * FROM @jobs AS j WHERE @jobName < j.job_name) BEGIN
  ...
  ,@totalSeconds = j.avg_runtime
   ...
   PRINT N'';
   PRINT N'WHILE EXISTS (SELECT * FROM @job_info as ji WHERE ji.job_id = ''' + @jobID + N''' AND ji.running <> 0)';
   PRINT N'BEGIN';
...
PRINT N'END TRY';
PRINT N'BEGIN CATCH';
PRINT N'    PRINT N''Error checking/running ' + quotename(@restoreJobName) + N' or ' + quotename(@copyJobName) + N'. Rolling back and quitting execution...'';';
PRINT N'    ROLLBACK TRANSACTION;';
PRINT N'    RETURN;';
PRINT N'END CATCH;';
PRINT N'';
