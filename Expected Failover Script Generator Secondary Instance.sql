...
WHILE EXISTS(SELECT * FROM @databases AS d WHERE d.database_name > @databaseName) BEGIN
   ...
   PRINT N'';
   PRINT N'	SET @start = current_timestamp' -- Use to filter monitor_history_detail (MHD);
   PRINT N'';
   ...
   PRINT N'          log_time > @start;';
   PRINT N'';
   PRINT N'       raiserror(''during: %s'',0,1,@afterLogCount) WITH NOWAIT; --flush print buffer';
   PRINT N'';
   PRINT N'	END;';
   PRINT N'';
   ...
   PRINT N'          @session = session_status'; 
   PRINT N'       FROM';
   PRINT N'          log_shipping_monitor_history_detail';
   PRINT N'       ORDER BY'; 
   PRINT N'          log_time DESC';
   PRINT N'';
   PRINT N'';
   ...
   PRINT N'          log_time DESC';
   PRINT N'';
   PRINT N'       raiserror(''Session: %s'',0,1,@session) WITH NOWAIT; --flush print buffer'
   PRINT N'';
   PRINT N'	END;';
   PRINT N'';
   ...
   PRINT N'	ELSE BEGIN';
   PRINT N'	   PRINT N'''';';
   PRINT N'	   PRINT N''' + quotename(@copyJobName) + N' quit with warning/failure. Rolling back and quitting batch execution...'';';
   PRINT N'       PRINT N'''';';
   PRINT N'	   ROLLBACK TRANSACTION;';
   PRINT N'       RETURN;';   
   PRINT N'	END;';
   PRINT N'';
   PRINT N'    -- Wait for the logshipping tables to update the last_copied_file';
   PRINT N'';
   ...
   PRINT N'';
   PRINT N'	PRINT N''Initial Restore: '' + @initialRestore;'; --Testing   
   PRINT N'';
   PRINT N'	SET @start = CURRENT_TIMESTAMP -- Used to filter monitor_history_detail (MHD)';
   PRINT N'';
   PRINT N'	-- get the initial count(*) for MHD';
   PRINT N'';
   PRINT N'	SELECT ';
   PRINT N'	   @beforeLogCount = Count(*)'; -- Could just set to 0
   PRINT N'	FROM ';
   PRINT N'	   log_shipping_monitor_history_detail';
   PRINT N'    WHERE';
   PRINT N'       log_time > @start;';
   PRINT N'';
   PRINT N'	PRINT N''Initial MHD Count: '' + @beforeLogCount;'; --Testing
   PRINT N'';
   ...
   PRINT N'        END;'
   PRINT N'';
   PRINT N'    -- We check to make sure necessary tables are updated before checking that the restore job applied the log tail backup';
   PRINT N'';
   PRINT N'	-- get the current count(*) for MHD';
   PRINT N'';
   PRINT N'	SELECT';
   ...
   PRINT N'       log_time > @start;';
   PRINT N'';
   PRINT N'	PRINT N''After Job MHD Count: '' + @afterLogCount;'; -- Testing
   PRINT N'';
   PRINT N'	-- wait for MHD to get updated';
   PRINT N'';
   PRINT N'	WHILE(@afterLogCount = @beforeLogCount)BEGIN';
   ...
   PRINT N'          log_time > @start;';
   PRINT N'';
   PRINT N'       raiserror(''MHD Count During: %s'',0,1,@afterLogCount) WITH NOWAIT; --flush print buffer';
   PRINT N'';
   PRINT N'	   WAITFOR DELAY ''00:00:1'';';
   PRINT N'';
   PRINT N'	END;';
   ...
   PRINT N'       log_time DESC';
   PRINT N'';
   PRINT N'	-- A log with a session of 0 or 1 means that the job hasn''t completed yet';
   PRINT N'';
   PRINT N'	WHILE(@session = 0 OR @session = 1)BEGIN';
   PRINT N'';
   PRINT N'	   WAITFOR DELAY ''00:00:1'';';
   ...
   PRINT N'	   log_shipping_monitor_history_detail';
   PRINT N'';
   PRINT N'	PRINT N''Final Log Count: '' + @afterLogCount;';
   PRINT N'';
   PRINT N'	PRINT N''Elapsed Time: '' + CAST(DATEDIFF(s, @start, @stop) AS VARCHAR);';
   PRINT N'';
   ...
   PRINT N'';
   PRINT N'	if(@session = 2)BEGIN -- 2 means the job ran successfully';
   PRINT N'';
   PRINT N'	   -- The # of logs restored isn''t as simple to access as it was for the copy jobs, as it is one of the potentially numerous session_status = 1 (job is running) rows';
   ...
   PRINT N'	ELSE BEGIN';
   PRINT N'	   PRINT N'''';';
   PRINT N'	   PRINT N''' + quotename(@restoreJobName) + N' quit with warning/failure. Rolling back and quitting batch execution...'';';
   PRINT N'       PRINT N'''';';
   PRINT N'	   ROLLBACK TRANSACTION;';    
   PRINT N'       RETURN;';
   PRINT N'	END;';
   PRINT N'';
   PRINT N'    -- If the number of files restored is nonzero, we check to see if the log tail backup was restored';
   PRINT N''
   PRINT N'    IF(CAST(@numRestored AS INT) > 0)BEGIN';
   PRINT N'';
   PRINT N'         -- Since we know that the last_copied_file is the log tail backup, we check the last_restored_file against it to ensure that it was applied';
   PRINT N'';
   PRINT N'		IF (EXISTS(';
   PRINT N'		   SELECT';
   PRINT N'		 	 *';
