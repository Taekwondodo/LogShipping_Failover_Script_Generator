 ...
 * 
 * Params:
 *  - @databaseName = Specifies the name of a single database, or, if null, to list all primary logshipped databases
 *
 */
...
print N'/*';
print N' * *****RUN ON ' + quotename(@secondaryServer) + N' AFTER SETTING UP PRMARY LOGSHIPPING ON ' + quotename(@@SERVERNAME) + N'*****';
   ...
   */
   ...
   if (@principal_name is null) begin
      ...
      where    (dp.[sid] <> 0x01) -- don''t want sa/dbo
               and (dp.name <> ''dbo''); -- added this as the above wasn''t filtering out dbo
